...
from diff.modules import UNet_conditional, EMA
...
class Diffusion:
    def __init__(self, noise_steps=10, beta_start=1e-4, beta_end=0.02, img_size=32, num_classes=10, c_in=3, c_out=3, device="cuda:0", **kwargs):
    ...
    def prepare(self, epochs,lr):
        # mk_folders(args.run_name)#najafi
        # self.train_dataloader, self.val_dataloader = get_data(args)#najafi
        self.optimizer = optim.AdamW(self.model.parameters(), lr=lr, eps=1e-5)
        self.scheduler = optim.lr_scheduler.OneCycleLR(self.optimizer, max_lr=lr, steps_per_epoch=50, epochs=epochs)
    ...
    def fit(self,epochs,do_validation,log_every_epoch,run_name):
        ...
        # save model
        self.save_model(run_name=run_name, epoch=epoch)
