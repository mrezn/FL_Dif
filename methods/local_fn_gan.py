...
device = torch.device("cuda")#cuda:0" if torch.cuda.is_available() else "cpu")
max_norm = 10
...
def creat_label_from_onehot(y_onehot):
    ...
    labels=torch.ones(batch_sizee).long().cuda()
...
def train_global_model_cgan_bs_weight_iloop_fz_dis(s_model, g_model, t_model, client_params,
    ...
    t_model.to(device)
    # print(g_model)
    g_model.device_to(device) #najafi


    # tensor_x = torch.Tensor(trn_x) #najafi
    # tensor_y = torch.Tensor(trn_y)#najafi

    # my_dataset = TensorDataset(tensor_x,tensor_y) #najafi
    # my_dataloader = DataLoader(my_dataset)#najafi
    ...
    g_parameters=g_model.get_parameters() #najafi 
    optimizer_G =  torch.optim.AdamW(g_parameters, lr=gen_lr, eps=1e-5)
    scheduler_G = torch.optim.lr_scheduler.OneCycleLR(optimizer_G, max_lr=gen_lr, steps_per_epoch=50, epochs=25)
    # self.scheduler = optim.lr_scheduler.OneCycleLR(self.optimizer, max_lr=lr, epochs=epochs)# steps_per_epoch=len(self.train_dataloader)#najafi
    scaler_G= torch.cuda.amp.GradScaler()

    # optimizer_G = torch.optim.Adam(g_parameters, lr=gen_lr, eps=1e-5)
    # scheduler_G = torch.optim.lr_scheduler.OneCycleLR(optimizer_G, max_lr=gen_lr, epochs=25)

    # optimizer_G = torch.optim.Adam(g_parameters, lr=gen_lr) #najafi
    iterations = 10
    inner_round_g = 1
    inner_round_d = 5
    ...
    for e in range(iterations):
        ...
        y_onehot_oneD=creat_label_from_onehot(y_onehot)#najafi
        z = torch.randn((batch_size, nz, 1, 1)).cuda() #najafi
        ...
        for _ in range(inner_round_g):
            for client in range(num_clients):
                optimizer_G.zero_grad()#najafi
                t_model = set_client_from_params(t_model, client_params[client])
                loss, loss_md, loss_cls, loss_ap = compute_backward_flow_G_dis(z, y_onehot_oneD, labels, g_model,
                                                                                    s_model, t_model,
                ...
                # g_model.train_step(loss)#najafi
                # optimizer_G.step()#najafi
        ...
        for _ in range(inner_round_d):
            ...
            # fake=sample(z, y_onehot , g_model) #najafi
            # time=torch.randint(0,1000,(3,)).cuda()#najafi
            # y_onehot_=y_onehot.unsqueeze(-1).unsqueeze(-1)#najafi
            fake= g_model.sample(use_ema=False, labels=y_onehot_oneD)#najafi
            fake = fake.float()
            # fake = g_model(z, y_onehot).detach()#najafi
...
def get_mdl_nonbn_idx(model_list, name_par=None):
    ...
    for i, mdl in enumerate(model_list):
        idx = 0
        for name, param in dict(mdl.state_dict()).items():
            temp = param.data.cuda().numpy().reshape(-1)
            if name in name_par:
...
def train_FedGAN_common(data_obj, act_prob, learning_rate, batch_size, epoch,
    ...
    glb_model_lr = 0.1
    gen_model_lr = 0.01
    # init_g_model = init_g_model.device_to(device)
    # init_g_model.prepare( 25 , gen_model_lr)
    ...
    if trial or (not os.path.exists('%sModel/%s/%s/%dcom_sel.pt' % (data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            avg_model = set_client_from_params(model_func(), 
                                               np.sum(clnt_params_list[selected_clnts] * weight_list[selected_clnts] / np.sum(weight_list[selected_clnts]),
                axis=0))
